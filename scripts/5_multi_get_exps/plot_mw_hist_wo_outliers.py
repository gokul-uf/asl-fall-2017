...
def plot_hist(mean_bin, std_bin, shard_mode):
	...
	plt.title("Distribution of Response Times(w/o outliers) at MW, Sharding: {}".format(shard_mode))
	plt.bar(left = vals, height = means)
	plt.xticks(range(11))
	plt.savefig("5_mw_hist_no-outlier_shard_{}.png".format(shard_mode))

	plt.figure()
	plt.xlabel("Response Time (msec)")
	plt.ylabel("log(Num Requests)")

	plt.title("Distribution of log(Response Times) w/o outliers at MW, Sharding: {}".format(shard_mode))
	h = []
	for x in means:
		if x < 1:
			x *= 4.
	...
	plt.savefig("5_mw_log_hist_no-outlier_shard_{}.png".format(shard_mode))
