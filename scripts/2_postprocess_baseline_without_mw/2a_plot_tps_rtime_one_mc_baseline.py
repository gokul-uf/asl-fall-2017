...
with open("csvs/one_mc_baseline_r-o_rt.csv") as f:
	for line in f:
...
val    = [] 
std    = []
with open("csvs/one_mc_baseline_w-o_rt.csv") as f:
	for line in f:
		vc, v, s = [float(x) for x in line.strip().split(",")][:3]
		num_vc.append(vc)
		val.append(v)
...
with open("csvs/one_mc_baseline_r-o_tps.csv") as f:
	for line in f:
		vc, v, s = [float(x) for x in line.strip().split(",")][:3]
		num_vc.append(vc)
		val.append(v)
...
val    = [] 
std    = []
with open("csvs/one_mc_baseline_w-o_tps.csv") as f:
	for line in f:
		vc, v, s = [float(x) for x in line.strip().split(",")][:3]
		num_vc.append(vc)
		val.append(v)
...
plt.grid(linestyle = "dotted")
# plt.ylabel("Throughput (in 100s reqs/sec)")
